...
from crstlmeth.web.utils import list_builtin_kits, preserve_tabix_pair_timestamp
...
def _collect_extra_bedmethyl(files: list | None) -> list[Path]:
    ...
    # Repair timestamps after saving all files, because .gz and .tbi may be
    # uploaded in either order.
    for p in persisted:
        if p.name.endswith(".bedmethyl.gz"):
            preserve_tabix_pair_timestamp(p)

    # Ensure indexing only if no .tbi was uploaded.
    for p in persisted:
        if p.name.endswith(".bedmethyl.gz"):
            _ensure_tabix_index(p)

    # Return only gz files as inputs to methylation calls.
    return [p for p in persisted if p.name.endswith(".bedmethyl.gz")]
