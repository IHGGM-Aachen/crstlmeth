...
from crstlmeth.core.samples import sample_status_table
...
def _scan() -> None:
    ...
    with st.spinner("scanning folders ..."):
        # Keep incomplete samples visible; analysis pages display index readiness.
        bed_by_sample = (
            scan_bedmethyl(data_path, require_index=False) if data_path else {}
        )
        cmeth_files = [
...
with st.container(border=True):
    ...
    scan_clicked = st.button("scan folders", type="primary", width="stretch")
...
if scan_clicked or auto_scan:
    ...
    if auto_scan:
        st.toast("Scanned folders from config defaults")
...
n_hap_resolved = (
    int(status_df["ready_haps"].sum()) if not status_df.empty else 0
)
n_pooled_only = (
    int((status_df["ready_ungrouped"] & ~status_df["ready_haps"]).sum())
    if not status_df.empty
    else 0
)
n_incomplete = (
    int((~status_df["ready_any"]).sum()) if not status_df.empty else 0
)
...
with r1c3:
    ...
    box.caption("all discovered *.bedmethyl.gz files")

with st.expander("sample detection table", expanded=False):
    if status_df.empty:
        st.info("No bedMethyl files detected yet.")
    else:
        st.caption(
            "Index columns show whether the matching .tbi is present. "
            "Files without .tbi are shown here but are not ready for plotting."
        )
        st.dataframe(status_df, width="stretch", hide_index=True)
        if n_incomplete:
            st.warning(
                f"{n_incomplete} sample(s) have bedMethyl files but no usable tabix index yet."
            )
...
- go to **references** to create CMETH cohort reference files.
""")
