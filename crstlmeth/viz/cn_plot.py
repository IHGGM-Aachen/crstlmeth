...
def _numeric_column_1d(df: pd.DataFrame, column: str) -> np.ndarray:
    """Return a single numeric column as a 1D float array.

    Be defensive about duplicate column names: pandas returns a DataFrame
    rather than a Series when duplicate labels are present. Use the first
    duplicate so plotting code receives scalars, not per-row arrays.
    """
    values = df[column]
...
def plot_cn_from_quantiles(
    ...
    *,
    ...
    # keep rows with q50. Use a defensive column accessor because duplicate
    # quantile column names in older intermediate references would otherwise
    # make q["q50"] a DataFrame instead of a Series.
    q50_for_mask = _numeric_column_1d(q, "q50")
    mask = np.isfinite(q50_for_mask)
    regions_used = [
        r for r, m in zip(regions, mask, strict=False) if m
    ] or list(regions)
    q = q.loc[regions_used]

    q25 = _numeric_column_1d(q, "q25")
    q50 = _numeric_column_1d(q, "q50")
    q75 = _numeric_column_1d(q, "q75")
    q10 = _numeric_column_1d(q, "q10") if "q10" in q.columns else None
    q90 = _numeric_column_1d(q, "q90") if "q90" in q.columns else None
